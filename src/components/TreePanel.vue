...
Issues with async loading:
  Somehow when the tree is continously rendered while the treeData array is generated, selecting a file/folder in the tree afterwards does not automatically select all children files/folder.
  ...
  <div>
    <div class="column tree">
      ...
      <div>
        ...
        <v-jstree v-if="show" class="jstree" ref="tree" :data="treeDataFolders" show-checkbox multiple allow-batch whole-row @item-click="itemClick"></v-jstree>
        <!-- ASYNC LOADING:
        <v-jstree v-if="show" class="column jstree" ref="tree" :data="asyncData" :async="loadData" show-checkbox multiple allow-batch whole-row @item-click="itemClick"></v-jstree> -->
      </div>
    </div>
    <div class="column folderPanel">
      <FolderPanel @push-clicked="push" :treeDataProp="treeData" :showProp="show" /> <!-- This passes the globalTreeData as the variable treeStructure down to the tree component (which is a child of this component) - Source: https://www.smashingmagazine.com/2020/01/data-components-vue-js/#propos-share-data-parent-child-->
    </div>
  </div>
</template>
...
<script>
  ...
  let folderIndex = 0; // The index of the array of the currently processed object
  let contents;
  ...
  export default {
    ...
    methods: {
      ...
      async getFiles(folder, parentId = null, path = [], folderPath = []) {
        ...
        for await (const entry of folder.values()) {
          ...
          if (entry.kind === "file") {
            ...
            */
            ...
            await this.assign(treeDataFolders, folderPath, {
              id: id,
              text: entry.name,
              value: "",
              icon: "",
            ...
            */
            ...
            //Get index of the current folder entry
            await this.getFolderIndex(treeDataFolders, folderPath, id)
            // Creating the path of this directory
            await this.moveFolderDown(folderPath)
            ...
            await this.getFiles(entry, id, path, folderPath)
          }
        }
        ...
        await this.moveFolderUp(folderPath)
      },
      ...
      async getFolderIndex(obj, keyPath, id) {
        
        let keyPathLength = keyPath.length

        if (keyPathLength < 2) {
          folderIndex = obj.findIndex(x => x.id === id)
        }
        else {
          let lastKeyIndex = keyPath.length - 1;
          for (var i = 0; i < lastKeyIndex; ++ i) {
          ...
          folderIndex = obj[keyPath[lastKeyIndex]].findIndex(x => x.id === id)
        }
      },
      ...
      async moveFolderDown(folderPath) {
            folderPath.push(folderIndex); // parentIndex = 0 is the initial state when the function is first run (hence at this part the first directory is opened)
            folderPath.push('children');
      },
      ...
      async moveFolderUp(folderPath) {
        let arrayLength = folderPath.length
        ...
        */
        ...
        if (arrayLength > 0) {
          let newArrayLength = arrayLength - 2
          folderPath.length = newArrayLength
        }
      },
